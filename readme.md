...
1. Скопируйте файл окружения и при необходимости обновите значения:
   ```bash
   cp .env.example .env
   ```
2. Установите зависимости (их нет, но команда создаст `package-lock.json` и проверит версию npm):
   ```bash
   npm install
   ```
